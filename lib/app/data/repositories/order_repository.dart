...
class OrderRepository {
  ...
  Future<List<OrderData>> getOrderHistory(
    int page, {
    String? customerName = "",
  }) async {
    try {
  ...
  Future<OrderData?> getOrderDetail(int orderId) async {
    try {
      final ResponseModel<OrderDetailResponse> responseModel =
          await ResponseModel.fromApiResponse<OrderDetailResponse>(
        () async => await OrderApiService().getOrderDetail(orderId),
        (body) => OrderDetailResponse.fromJson(body),
      );
      if (responseModel.status == ResponseStatus.success) {
